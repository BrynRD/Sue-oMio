...
export default function ProductosPage() {
  ...
  const [filterTallas, setFilterTallas] = useState<string[]>([])
  ...
  // Extraer colores y tallas disponibles cuando cargan los productos
  useEffect(() => {
    if (productos.length > 0) {
      ...
      productos.forEach(producto => {
        // Extraer colores de imagenes_por_color
        if (producto.imagenes_por_color) {
          Object.keys(producto.imagenes_por_color).forEach(color => colores.add(color))
        }
      ...
      setColoresDisponibles(Array.from(colores))
      setTallasDisponibles(Array.from(tallas))
    }
  }, [productos])
  ...
  // Función para cambiar color seleccionado en el producto
  ...
  // Función para manejar filtros de color
  ...
  // Función para manejar filtros de talla
  const handleTallaFilter = (talla: string) => {
    setFilterTallas(prev => 
      prev.includes(talla) 
  ...
  // Función para manejar filtros de disponibilidad
  const handleDisponibilidadFilter = (tipo: string) => {
    setFilterDisponibilidad(prev => 
      prev.includes(tipo) 
        ? prev.filter(d => d !== tipo)
        : [...prev, tipo]
    )
  }
  ...
  const filteredProducts = useMemo(() => {
    ...
    return productos.filter(producto => {
      ...
      const matchesColor = filterColors.length === 0 || 
        (producto.imagenes_por_color && filterColors.some(color => 
          Object.keys(producto.imagenes_por_color!).includes(color)
        ))
      
      // Filtro por tallas
      const matchesTalla = filterTallas.length === 0 || 
        (producto.variantes && filterTallas.some(talla =>
          producto.variantes!.some(variante => variante.talla === talla)
        ))
      
      // Filtro por disponibilidad
      const matchesDisponibilidad = filterDisponibilidad.length === 0 || 
        filterDisponibilidad.some(tipo => {
          if (tipo === 'en-stock') return producto.stock > 0
          if (tipo === 'agotado') return producto.stock === 0
          return true
        })
      
      return matchesSearch && matchesCategory && matchesPrice && matchesColor && matchesTalla && matchesDisponibilidad
    })
  }, [productos, searchTerm, selectedCategory, priceRange, filterColors, filterTallas, filterDisponibilidad, getPrecioProducto])
          ...
          <div className="w-64 flex-shrink-0">
            <div className="sticky top-8">
              <Card className="p-6">
                ...
                {/* Colores - Solo mostrar los que existen en la BD */}
                  ...
                  <div className="mb-6">
                    ...
                    {filterColors.length > 0 && (
                      <div className="mt-2 text-xs text-stone-600">
                        Filtrando por: {filterColors.join(', ')}
                      </div>
                    )}
                  </div>
                )}

                {/* Tallas - Solo mostrar las que existen en la BD */}
                    ...
                    <div className="space-y-2">
                      {tallasDisponibles.map(talla => (
                        <div key={talla} className="flex items-center space-x-2">
                          <Checkbox 
                            id={`talla-${talla}`}
                            checked={filterTallas.includes(talla)}
                            onCheckedChange={() => handleTallaFilter(talla)}
                          />
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center space-x-2">
                      <Checkbox 
                        id="en-stock"
                        checked={filterDisponibilidad.includes('en-stock')}
                        onCheckedChange={() => handleDisponibilidadFilter('en-stock')}
                      />
                    ...
                    <div className="flex items-center space-x-2">
                      <Checkbox 
                        id="agotado"
                        checked={filterDisponibilidad.includes('agotado')}
                        onCheckedChange={() => handleDisponibilidadFilter('agotado')}
                      />
